...
- リアルタイム `/metrics` (Prometheus): 生成/プロンプト tok/s、処理中/
  待機中リクエスト、デコード毎の使用中スロット数、プロンプト・生成の累計カウンタ
...
- **インラインガイダンス**: 各カードの ⓘ ツールチップでパラメータを説明し、
  状態が閾値を超えると推奨アクションを表示
- **チャットパネル内蔵**: `POST /v1/chat/completions` のストリーミング、
  システムプロンプト、パラメータスライダー、停止ボタン、スロット負荷試験用の
  並列ファンアウトモード(下記参照)
...
スロットグリッドとモデルカードの間に、折りたたみ可能なチャットパネルがあります。
監視中の同じ llama-server に対して話しかけるので、プロンプトを送って
メトリクスがリアルタイムにどう反応するかを観察できます。

- **ストリーミング**: `/v1/chat/completions` の SSE をトークン単位で表示。
  停止ボタンで進行中のストリームを abort。
- **Markdown レンダリング**: 見出し、段落、**太字**、*斜体*、インライン
  `code`、コードブロック、順序付き/順序なしリスト、表 (GFM)、引用、
  水平線、`[text](https://…)` リンクをサポート。外部ライブラリ不使用、
  すべてのテキストは `textContent` で挿入されるためサーバー応答が
  HTML / スクリプトを注入することはできません。
- **システムプロンプト**: 任意。`role: "system"` として履歴の前に送信。
- **スライダー**: `temperature`、`top_p`、`max_tokens`、そして同じ
  プロンプトを N スロットに同時発射する並列ファンアウト (1–8) — 
  簡易飽和テスト用。
- **永続化なし**: プロジェクトのルールにより localStorage は無効、
  リロードで会話は消えます。URL からプロンプトを渡すには `?prompt=…` を使用。
...
| シグナル | しきい値 | 提案 |
|---|---|---|
| デコード毎の使用中スロット | total_slots の 90% 以上が継続 | `--parallel` を増やす、またはクライアント並行度を下げる |
